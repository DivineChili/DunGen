...
void Cell::toggleSubCellSide(int side, bool state){
	...
	case UP:
		for (int i = 0; i < 5; i++) {
			if (state) {
				...
				this->subCell_grid[0][i] = new Door(i, 0);
			}
		}
		break;
	case LEFT:
		for (int i = 0; i < 5; i++) {
			if (state) {
				this->subCell_grid[i][0] = new Wall(0, i);
			}
			else {
	...
	case DOWN:
		for (int i = 0; i < 5; i++) {
			if (state) {
				this->subCell_grid[4][i] = new Wall(4, i);
			}
			else {
				this->subCell_grid[4][i] = new Door(4, i);
			}
		}
		break;
	case RIGHT:
		for (int i = 0; i < 5; i++) {
			if (state) {
				...
				this->subCell_grid[i][4] = new Door(i, 4);
			}
		}
		break;
	}
